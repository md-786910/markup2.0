...
exports.proxyPage = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Retry via fallback proxy for connection-blocked domains (ETIMEDOUT/ECONNREFUSED)
    if (['ETIMEDOUT', 'ECONNREFUSED'].includes(err.code) && !req.query._fallback) {
      try {
        console.log(`Direct connection failed for ${url} (${err.code}), trying fallback proxy...`);
        ...
        console.log(`Fallback proxy succeeded for ${url}`);
        return processResponse(req, res, fallbackResp, url, projectId, serverBase);
      } catch (fallbackErr) {
        console.error(`Fallback proxy also failed for ${url}: ${fallbackErr.message}`);
      }
    }
