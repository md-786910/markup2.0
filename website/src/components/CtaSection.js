...
export default function CtaSection() {
  return (
    <section className="relative overflow-hidden bg-[#101626] py-20 text-white sm:py-24">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(40,84,255,0.28),transparent_34%),radial-gradient(circle_at_85%_70%,rgba(143,209,79,0.14),transparent_30%)]" />
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-4 text-center sm:px-6">
        <span className="inline-flex rounded-full border border-white/10 bg-white/8 px-3 py-1 text-xs font-semibold uppercase text-white/70">
          Ready when your next review starts
        </span>
        <h2 className="mt-6 font-display text-3xl font-bold leading-tight tracking-tight text-white md:text-5xl">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-sm text-slate-400">
          {['No credit card required', 'Guest links included', 'Cancel any time'].map((item) => (
            <span key={item} className="inline-flex items-center gap-2">
              <CheckIcon className="h-4 w-4 text-[#8fd14f]" />
              {item}
            </span>
          ))}
        </div>
