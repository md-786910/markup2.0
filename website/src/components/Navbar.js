      ...
      <div className={scrolled ? 'h-[104px]' : ''}>
        <div className="bg-[#111521] text-white">
          <div className="site-container flex h-10 items-center justify-center gap-3 text-center text-[13px] font-medium">
            <span className="inline-flex h-5 w-5 items-center justify-center rounded-lg bg-[#8fd14f] text-[11px] font-bold text-[#111521]">
              3
            </span>
            <span className="hidden text-white/80 sm:inline">New: realtime cursors, instant video notes, and controlled guest reviews.</span>
            <a href="/#features" className="font-semibold text-white underline-offset-4 hover:underline">
              See what changed
            </a>
          </div>
        </div>
        <header className={scrolled ? 'fixed inset-x-0 top-0 z-50' : 'relative z-50'}>
          <nav className={`border-b bg-white/92 backdrop-blur-xl transition-all duration-300 ${scrolled ? 'border-slate-200 shadow-[0_12px_34px_rgba(15,23,42,0.07)]' : 'border-white/60'}`} aria-label="Main navigation">
            <div className="site-container flex h-16 items-center justify-between">
              <div className="flex min-w-0 flex-1 items-center">
                <Link to="/" className="flex items-center gap-3">
                  <span className="grid h-9 w-9 place-items-center rounded-xl bg-[#2854ff] text-white shadow-[0_10px_24px_rgba(40,84,255,0.26)]">
                  ...
                  <span className="font-display text-[1.45rem] font-semibold tracking-tight text-[#1a1f2e]">Markly</span>
                </Link>
              </div>

              <div className="hidden flex-1 items-center justify-center gap-2 rounded-full border border-slate-200/70 bg-white/70 p-1 md:flex">
                {NAV_LINKS.map((link) => (
                  <Link
                    key={link.label}
                    to={link.href}
                    className="rounded-full px-4 py-2 text-[14px] font-semibold text-[#5c6475] transition-colors hover:bg-slate-50 hover:text-[#1a1f2e]"
                  >
                    {link.label}
                  </Link>
                ))}
              ...
              <div className="hidden flex-1 items-center justify-end gap-6 md:flex">
                ...
                <a
                  ...
                  className="inline-flex h-10 items-center rounded-full bg-[#2854ff] px-5 text-[14px] font-semibold text-white shadow-[0_10px_24px_rgba(40,84,255,0.24)] transition-transform hover:-translate-y-px"
                >
                  Sign up free
                </a>
              </div>
