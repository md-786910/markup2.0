...
export default function ProjectView({
  ...
  const [devicePinCounts, setDevicePinCounts] = useState({
    desktop: 0,
    tablet: 0,
    mobile: 0,
  });
  ...
  const getMemberTooltip = useCallback(
    (member) => {
  ...
  const proxyUrl = !isDocumentProject
    ? `${process.env.REACT_APP_BASE_URL || "http://localhost:5000/api"}/proxy?url=${encodeURIComponent(targetUrl)}&projectId=${project._id}&token=${token}`
    : null;

  const currentDoc = isDocumentProject
    ? project.documents?.[currentDocIndex] || project.documents?.[0]
    : null;
    ...
    getPinsApi(project._id)
      .then((res) => {
        ...
        for (const pin of res.data.pins) {
          if (
            pin.status === "pending" &&
            counts[pin.deviceMode || "desktop"] !== undefined
          ) {
  ...
  useEffect(() => {
    ...
    if (
      iframeState.screenshot &&
      pendingPinData &&
      !pendingPinData.screenshot
    ) {
      setPendingPinData((prev) =>
        prev ? { ...prev, screenshot: iframeState.screenshot } : prev,
      );
  ...
  useEffect(() => {
    const handler = (event) => {
      ...
      if (data?.type === "MARKUP_PIN_CLICK") {
        const pin =
          pins.find((p) => p._id === data.pinId) ||
          allPins.find((p) => p._id === data.pinId);
        if (pin) {
  ...
  const handleDeletePin = async (pinId) => {
    try {
      ...
      if (pin && pin.status === "pending") {
        ...
        setDevicePinCounts((prev) => ({
          ...prev,
          [mode]: Math.max(0, (prev[mode] || 0) - 1),
        }));
      }
  ...
  useEffect(() => {
    return onEvent("pin:created", (data) => {
      setAllPins((prev) => {
        ...
        if (data.pin.deviceMode && data.pin.deviceMode !== deviceMode)
          return prev;
  ...
  useEffect(() => {
    if (!initialPinId || allPins.length === 0 || deepLinkHandled.current)
      return;
  ...
  const handlePinNavigate = (pin) => {
    ...
    if (isDocumentProject) {
      ...
      if (parsed) {
        const docIdx = project.documents.findIndex(
          (d) => d.filename === parsed.filename,
        );
        if (docIdx >= 0 && docIdx !== currentDocIndex)
          setCurrentDocIndex(docIdx);
    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="bg-white border-b border-gray-200/80 px-4 h-14 flex items-center justify-between relative">
        ...
        <div className="flex items-center gap-3 min-w-0 z-10">
          ...
          <h2 className="font-semibold text-gray-900 shrink-0 text-[14px] tracking-tight">
            {project.name}
          </h2>
          <div
            className="flex items-center gap-1.5 text-xs text-gray-400 bg-gray-50/80 pl-2 pr-2.5 py-1.5 rounded-lg truncate max-w-md border border-gray-100/80 hover:bg-gray-100/60 transition-colors cursor-default"
            title={
              isDocumentProject ? currentDoc?.originalName : currentPageUrl
            }
          >
            {isDocumentProject ? (
              <svg
            ...
            <span className="truncate">
              {isDocumentProject
                ? `${currentDoc?.originalName} — Page ${currentDocPage}`
                : currentPageUrl}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 z-10">
          ...
          {canCreate && (
            <div className="relative flex bg-gray-100/80 rounded-lg p-1">
              <div
              ...
              <button
                onClick={() => {
                  ...
                  setPinMode(false);
                  setTimeout(() => setModeSwitching(false), 400);
                }}
          ...
          <div className="flex items-center -space-x-2">
            {allMembers.slice(0, 5).map((member) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full ${color.bg} ${color.text} flex items-center justify-center text-[11px] font-bold border-[2.5px] border-white`}
                  >
            ...
            >
              ...
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-4.874a4.5 4.5 0 00-1.242-7.244l4.5-4.5a4.5 4.5 0 016.364 6.364l-1.757 1.757"
                />
              </svg>
              Share
            </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {sidebarTab === "pins" ? (
          ...
          <div className="w-80 shrink-0 bg-white border-r border-gray-200 flex flex-col h-full overflow-hidden">
            ...
            {sidebarTab === "activity" && (
              <ActivitySidebar projectId={project._id} />
            )}
            {sidebarTab === "versions" && (
              <VersionSidebar
                projectId={project._id}
      ...
      {pendingPinData && (
        <NewPinCommentPopup
          ...
          onClose={() => {
            setPendingPinData(null);
            if (!isDocumentProject) iframeState.clearScreenshot();
            setLastCreatedPinId(null);
          }}
          onPinCreated={(pinId) => {
            loadPins();
