...
import { createCheckoutSessionApi, upgradePlanApi, verifyPaymentApi } from '../../services/billingService';
...
export default function UpgradeModal({ selectedPlan, currentPlanId, currentPlanName, onClose, onUpgradeComplete }) {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      if (err.response?.status === 400 && err.response?.data?.message?.includes('Razorpay plan not configured')) {
        try {
