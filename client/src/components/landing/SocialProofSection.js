        ...
        <div className="grid grid-cols-1 md:grid-cols-6 lg:grid-cols-12 gap-6 items-start">
          {testimonials.map((t, i) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className={`relative p-8 lg:p-10 rounded-[32px] bg-white border border-gray-100 transition-all hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] ${i === 0 ? 'md:col-span-6 lg:col-span-7' :
                i === 1 ? 'md:col-span-3 lg:col-span-5' :
                  'md:col-span-3 lg:col-span-12 lg:max-w-xl'
                }`}
            >
              <Quote className={`w-10 h-10 mb-6 opacity-10 ${t.color === 'brand' ? 'text-brand-600' :
                t.color === 'indigo' ? 'text-indigo-600' : 'text-blue-600'
                }`} />

              <p className="text-md lg:text-xl font-bold text-gray-800 leading-tight mb-10 tracking-tight">
                "{t.quote}"
              </p>
