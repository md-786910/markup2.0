...
export default function CreateProjectModal({ isOpen, defaultType = 'website', onClose, onCreated }) {
  const [projectType, setProjectType] = useState(defaultType);

  // Sync with defaultType when modal opens
  React.useEffect(() => {
    if (isOpen) setProjectType(defaultType);
  }, [isOpen, defaultType]);
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Create New Project">
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <button
          ...
          className={`relative flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all duration-150 hover:scale-[1.02] ${
            projectType === 'website'
        ...
        >
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${
            projectType === 'website' ? 'bg-blue-100' : 'bg-gray-100'
          }`}>
            <svg className={`w-5 h-5 ${projectType === 'website' ? 'text-blue-600' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-1.605.42-3.113 1.157-4.418" />
            </svg>
          </div>
          <span className={`text-sm font-semibold ${projectType === 'website' ? 'text-blue-700' : 'text-gray-700'}`}>Website</span>
          <span className="text-[11px] text-gray-400 leading-tight">Live URL to review</span>
          {projectType === 'website' && (
        ...
        >
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${
            projectType === 'document' ? 'bg-violet-100' : 'bg-gray-100'
          }`}>
            <svg className={`w-5 h-5 ${projectType === 'document' ? 'text-violet-600' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
            </svg>
          </div>
          <span className={`text-sm font-semibold ${projectType === 'document' ? 'text-violet-700' : 'text-gray-700'}`}>Document</span>
          <span className="text-[11px] text-gray-400 leading-tight">PDF or images to review</span>
          {projectType === 'document' && (
